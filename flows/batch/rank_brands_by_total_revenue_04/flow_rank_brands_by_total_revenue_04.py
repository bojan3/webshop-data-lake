...
@task
def sum_revenue_by_brand_task(data_period: str):
    return trigger_spark_job("sum_revenue_by_brand", data_period)


@task
def publish_brand_revenue_rank_task(data_period: str):
    return trigger_spark_job("publish_brand_revenue_rank", data_period)


@flow(name="rank_brands_by_total_revenue_04")
def rank_brands_by_total_revenue_04(data_period: str):
    ...
    msg3 = sum_revenue_by_brand_task.submit(data_period, wait_for=[msg2])
    msg4 = publish_brand_revenue_rank_task.submit(data_period, wait_for=[msg3])
...
if __name__ == "__main__":
    args = parse_args()
    rank_brands_by_total_revenue_04(args.data_period)
