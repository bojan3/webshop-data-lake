...
CLICKHOUSE_TABLE = "electronics_brand_units_sold_rank_09"
...
def publish_electronics_brand_units_sold_rank(data_period: str) -> None:
    processing_brand_units_sold_path = (
        f"hdfs://namenode:9000/data/processing/electronics_brand_units_sold/{data_period}"
    )

    spark = create_spark_session()
    df = spark.read.option("header", "true").parquet(processing_brand_units_sold_path)
...
if __name__ == "__main__":
    args = parse_args()
    publish_electronics_brand_units_sold_rank(args.data_period)
